...
**順序を守る。** 段階 5 と同じく、公開 API の形は実装より先に決める。

1. **ベンチの合格条件を、測る前にこの節に書く**（下の「ベンチ」）。
   **測ってから書くと、観測に合わせた条件になる**
2. **公開 API の形を [ADR-0007](adr/0007-multi-input-aggregation.md) で決める。**
   複数入力にしたとたん、次の 3 つが同時に崩れる。どれも段階 5 で
   「入力が 1 つである」ことを前提に先送りしたもの
   - `Counter` のマージ（`Extend` か専用の `merge` か。`strict` が食い違う
     `Counter` 同士を合流させたらどうなるか）
   - **どのファイルの失敗かを言えない**（`CliErrorKind::Tally` が
     `#[error(transparent)]` で path を持たない。`LineError` の行番号は
     ファイルごとに 1 から数えるので、path が無いと行番号も意味を失う）
   - `tally_reader` が `Report` を返す形（`Report` は順位づけ済みで
     `limit` で切り詰めうるので、**マージできない**）
3. **並列化をどちらのクレートに置くかを決める。** `tally-core` はファイルを開かない
   （`lib.rs` の不変条件）。ファイル単位の並列化は開く側の関心事かもしれない

### 完了条件

**複数入力**

- `tally a.log b.log ...` が全ファイルを 1 つの集計に合流させる。
  引数が 0 個なら従来どおり標準入力を読む
- **引数の順序を入れ替えても、出力が同じバイト列になる**
  （`crates/tally/docs/output-format.md` の並び順の契約を並列化で崩さない）
- **並列に集計した結果が、逐次に集計した結果と一致する**ことを、
  入力の分割のしかたを変えた性質テスト（`proptest`）で示す。
  例を数個並べるだけでは、マージの結合則の破れを拾えない
- **失敗したファイルが分かる。** エラーの表示に path と行番号の両方が出る
- **複数のファイルが失敗したとき、どれを報告するかの規則**が
  `output-format.md` に書かれ、テストされている。
  **並列に回すと「最初に失敗したもの」は実行ごとに変わりうる**

**`Send` / `Sync`**

- **並列化のために境界へ足した `Send` / `Sync` の 1 つ 1 つについて、
  なぜ要るかを説明できる。** とくに述語 `keep` の境界
  （段階 4 で `Fn` にした理由と、ここで足すものとの関係）

**ベンチ（合格条件。測る前に書いた）**

**「速くなること」は合格条件にしない。** 確認するのは
**速くならない条件が予測どおりに現れるか**である。
ただし **速くなる条件（陽性対照）を 1 つ必ず含める** — それが無いと、
「速くならなかった」が「並列化が壊れている」と区別できない。

**測り方を固定する。**

- **総行数を固定し、それを 1・10・1000 ファイルに分割する。**
  ファイル数と一緒に仕事量まで変えると、何を測ったのか分からなくなる
- 逐次版と並列版を**同じ入力**で比べ、比（並列 ÷ 逐次）を見る
- **キーの取り出し方を 2 種類にする。** 行全体（安い）と JSON のフィールド
  （パースが重い）。CPU バウンドの度合いが変わる
- **測定環境を記録する。** コア数（性能コアと効率コアの内訳）、
  rayon のスレッド数、ファイルサイズ、ページキャッシュが温まっているか
- `divan` でベンチを書く

**予測（外れても書き換えない。外れたことを `docs/stage-log.md` に書く）**

| # | 条件 | 予測 | 役割 |
| --- | --- | --- | --- |
| B1 | 1 ファイル | **速くならない**（比 ≥ 0.95）。ファイル単位の並列化では仕事が分かれない | 速くならない条件 |
| B2 | 10 ファイル、JSON フィールド、キーの種類が少ない | **速くなる**。ただし比はコア数の逆数より大きい | 陽性対照 |
| B3 | B2 と同じだが、キーがほぼ全行で異なる | **B2 より伸びない。** マージの費用が集計の費用と同じ桁になる | 速くならない条件 |
| B4 | 1000 ファイル、1 ファイルあたり数行 | **B2 より伸びない。** 仕事が open/close と rayon の分配に支配される | 速くならない条件 |

**I/O バウンドの条件はこの表に入れていない。** ページキャッシュが温まった
状態で測ると、それは I/O ではなくメモリの読み出しを測っている。
**カリキュラムの当初の前提「I/O バウンドなら並列化は効かない」は、
キャッシュが冷えた状態でしか検証できない。** しかも SSD は並列の読み出しで
帯域が上がるので、**前提そのものが成り立たない可能性がある。**
冷えた状態での測定は下の「検討する価値のあるもの」に置く。

**共通**

- `cargo fmt --all --check` / `cargo lint` / `cargo t` / `cargo deny check` が通る
- `cargo test --workspace --doc` と rustdoc の警告検査が通る

### 検討する価値のあるもの（必須ではない）

- **キャッシュが冷えた状態での測定。** macOS なら `sudo purge` の後に
  `hyperfine` で CLI ごと測る。`divan` の反復の中ではキャッシュを冷やせない。
  **予測は立てられない**（上の理由）。測ったら結果をそのまま記録する
- **`std::thread::scope` とチャネルで同じものを書き、rayon と比べる。**
  ワークスティーリングが何を肩代わりしているかが見える
