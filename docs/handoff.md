...
3. **`Selector` の公開 API。ADR-0005 として書く（未着手）。**
   段階 5 の完了条件の先頭に置いてある。積み残しは 2 つ:
   - **`#[non_exhaustive]` + `Selector::new(key)`** は口頭で合意したが**文書がない**。
     **後から `#[non_exhaustive]` を付けるのは破壊的変更**なので、切り出しの前に入れる
   - **`strict` が `Selector` に属するべきかは、まだ議論していない。**
     「取り出せなかったときどうするか」は結果に対する方針であって、
     「どこからどう取るか」ではない。移す場合の代償は
     `docs/curriculum.md` 段階 5 の「着手前に決めること」に書いた
