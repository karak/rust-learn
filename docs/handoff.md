...
3. **`Selector` の公開 bool フィールドが増え続けている**（`ignore_case` / `strict`、
   段階 4 で `--filter` が加わる）。外部から構築できる公開 API なので、
   フィールド追加は破壊的変更になる。段階 5 の `tally-core` 分離時に
   `#[non_exhaustive]` かビルダーを検討すること。
