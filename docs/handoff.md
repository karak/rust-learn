...
- **実装は段階 4 まで完了。段階 5（モジュール・クレート設計）は未着手。**
...
1. **完了条件を先に `docs/curriculum.md` に書く**（段階 3・4 で有効だった手順）
2. **未決事項 3・4 を先に片付ける。** どちらも
   「`tally-core` を切り出すと公開 API になるので、その前に決めること」と
   書いてある。**段階 5 がその期限。**
...
3. **`Selector` の公開 API。** 方針は「`#[non_exhaustive]` + `Selector::new(key)`」で
   合意済みだが、**まだ ADR に落としていない。**
   **後から `#[non_exhaustive]` を付けるのは破壊的変更**なので、
   切り出しの前に入れる必要がある。エラー型（[ADR-0004](adr/0004-error-type-shape.md)）とは
   独立した判断なので、別 ADR にする。
   あわせて **`strict` が `Selector` に属するべきか**も問い直す価値がある
   （抽出の性質ではなく結果に対する方針なので、呼び出し側の判断にもできる）
4. **エラー型の形は [ADR-0004](adr/0004-error-type-shape.md) が正本。** 中間文書として
   論点を 7 つに分割してあり、**決定済み 2・未決定 4・未測定 1**。
   未決定のものは段階 5 の実装前に決めること。ここには状態だけ書く:
   - 決定済み: 共有文脈を `LineError` に括り出す / `#[non_exhaustive]` は付けない
   - 未決定: 失敗の単位で型を分けるか / `serde_json::Error` の公開 /
     CLI 固有のエラー型を持つか / hint を持つか
